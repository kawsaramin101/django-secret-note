...
def index(request):
    if request.method == "POST":
        ...
        if note_form.is_valid():
            ...
            return redirect(reverse('notes:note_detail', kwargs={'id': note.id}))
...
def note_detail(request, id):
    ...
    if not note.password_to_read:
        ...
        print(note.password_to_read, password_to_read)
        if password_to_read:
            if check_password(password_to_read, note.password_to_read):
                return render(request, 'notes/detail.html', {'note': note})
            else:
                messages.error(request, "Password is incorrect.")
                return redirect(reverse('notes:enter_password_to_read', kwargs={'id': note.id}))
        else:
            return redirect(reverse('notes:enter_password_to_read', kwargs={'id': note.id}))
            
            
def enter_password_to_read(request, id):
    if request.method == "POST":
        password_to_read = request.POST.get('password_to_read')
        if password_to_read:
            request.session['password_to_read'] = password_to_read
            return redirect(reverse('notes:note_detail', kwargs={'id': id}))
...
def note_edit(request, id):
    ...
    if not password_to_edit:
        return redirect(reverse('notes:enter_password_to_edit', kwargs={'id': note.id}))
    if check_password(password_to_edit, note.password_to_edit):
        if request.method == "POST":
            ...
            if note_form.is_valid():
                ...
                password_to_read = note_form.cleaned_data.get("password_to_read")
                if password_to_read:
                    request.session["password_to_read"] = password_to_রেয়াদ
...
def enter_password_to_edit(request, id):
    if request.method == "POST":
        password_to_edit = request.POST.get('password_to_edit')
        if password_to_edit:
            request.session['password_to_edit'] = password_to_edit
            return redirect(reverse('notes:note_edit', kwargs={'id': id}))
        messages.error(request, "Password can't be empty.")
